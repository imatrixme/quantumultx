...
// 参数获取
var Pin0 =
  mark0 && para1.indexOf('in=') != -1 ? para1.split('in=')[1].split('&')[0].split('+').map(decodeURIComponent) : null;
var Pout0 =
  mark0 && para1.indexOf('out=') != -1 ? para1.split('out=')[1].split('&')[0].split('+').map(decodeURIComponent) : null;
var Psfilter =
  mark0 && para1.indexOf('sfilter=') != -1 ? Base64.decode(para1.split('sfilter=')[1].split('&')[0]) : null; // script filter
var Preg =
...
var Psrename =
  mark0 && para1.indexOf('srename=') != -1 ? Base64.decode(para1.split('srename=')[1].split('&')[0]) : null; // script rename
...
var Ppolicy =
  mark0 && para1.indexOf('policy=') != -1 ? decodeURIComponent(para1.split('policy=')[1].split('&')[0]) : 'Shawn';
...
function Type_Check(subs) {
  ...
  if (subs.indexOf(html) != -1 && link0.indexOf('github.com' == -1)) {
    ...
    ((RuleK.some(RuleCheck) && subs.indexOf(html) == -1 && !/\[(Proxy|server_local)\]/.test(subs)) ||
      typeU == 'rule' ||
      para1.indexOf('dst=filter') != -1) &&
    typeU != 'nodes'
  ) {
    ...
    (subi.indexOf('tag=') != -1 && QuanXK.some(NodeCheck) && !/\[(Proxy|filter_local)\]/.test(subs)) ||
    typeU == 'list'
  ) {
...
function CDN(cnt) {
  ...
  cnt = cnt
    .join('\n')
    .replace(/https:\/\/raw.githubusercontent.com\/(.*?)\/(.*?)\/(.*)/gim, 'https://cdn.jsdelivr.net/gh/$1/$2@$3');
  return cnt;
}
...
function Rule_Policy(content) {
  ...
  if (RuleK1.some(RuleCheck1) && !RuleK.some(RuleCheck)) {
    ...
    return nn;
  } else if (cnt.length == 1 && !RuleK.some(RuleCheck) && cnt[0] != '' && cnt[0].indexOf('payload:') == -1) {
    // 纯域名/ip 列表
    ...
    if (list0[i].trim().length > 3 && !/\;|\/|\#/.test(list0[i][0])) {
      ...
      var tag0 =
        list0[i].indexOf('tag=') != -1
          ? list0[i].split(/\&*(emoji|udp|tf0|cert|rename|replace)\=/)[0].split('tag=')[1]
          : '';
...
function QX_TLS(cnt, Pcert0, PTls13) {
  ...
  if (cnt.trim().indexOf('shadowsocks') != 0) {
    //关闭非 ss/ssr 类型的 udp
    udp = 'udp-relay=false, ';
...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  ...
  if (obfs.indexOf('obfs=over-tls') != -1 || obfs.indexOf('obfs=wss') != -1) {
    var cert =
      Pcert0 != 0 || subs.indexOf('allowInsecure=1') != -1 ? 'tls-verification=false, ' : 'tls-verification=true, ';
...
function SSR2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(':').length <= 6) {
    ...
    pwd =
      'password=' +
      Base64.decode(cnt.split('/?')[0].split(':')[5].replace(/-/g, '+').replace(/_/g, '/')).split('\u0000')[0];
    ...
    if (cnt.indexOf('obfsparam=') != -1) {
      obfshost =
        cnt.split('obfsparam=')[1].split('&')[0] != ''
          ? 'obfs-host=' +
            Base64.decode(cnt.split('obfsparam=')[1].split('&')[0].replace(/-/g, '+').replace(/_/g, '/'))
              .split(',')[0]
              .split('\u0000')[0] +
            ', '
          : '';
    ...
    if (cnt.indexOf('protoparam=') != -1) {
      oparam =
        cnt.split('protoparam=')[1].split('&')[0] != ''
          ? 'ssr-protocol-param=' +
            Base64.decode(cnt.split('protoparam=')[1].split('&')[0].replace(/-/g, '+').replace(/_/g, '/'))
              .split(',')[0]
              .split('\u0000')[0] +
            ', '
          : '';
    ...
    tag =
      'tag=' +
      Base64.decode(cnt.split('remarks=')[1].split('&')[0].replace(/-/g, '+').replace(/_/g, '/')).split('\u0000')[0];
...
function SSD2QX(subs, Pudp, Ptfo) {
  ...
  for (var i in cnt.servers) {
    ...
    if (cnt.servers[i].plugin_options) {
      obfs =
        cnt.servers[i].plugin_options.split(';')[0] != null ? ', ' + cnt.servers[i].plugin_options.split(';')[0] : '';
      obfshost =
        cnt.servers[i].plugin_options.split(';')[1] != null ? ', ' + cnt.servers[i].plugin_options.split(';')[1] : '';
    }
...
function SVmess2QX(content) {
  ...
  var ipport = cnt.split(',')[1].trim() + ':' + cnt.split(',')[2].trim();
  var puname =
    cnt.indexOf('username') != -1 ? 'password=' + cnt.split('username')[1].split(',')[0].split('=')[1].trim() : '';
  var pmtd = 'method=aes-128-gcm';
  var ptls13 = paraCheck(cnt, 'tls13') == 'true' ? 'tls13=true' : 'tls13=false';
  var pverify =
    cnt.replace(/ /g, '').indexOf('skip-cert-verify=false') != -1 ? 'tls-verification=true' : 'tls-verification=false';
...
function Shttp2QX(content) {
  ...
  var ipport = cnt.split(',')[1].trim() + ':' + cnt.split(',')[2].trim();
  var puname =
    cnt.indexOf('username') != -1 ? 'username=' + cnt.split('username')[1].split(',')[0].split('=')[1].trim() : '';
  var pwd =
    cnt.indexOf('password') != -1 ? 'password=' + cnt.split('password')[1].split(',')[0].split('=')[1].trim() : '';
...
function YAMLFix(cnt) {
  ...
  if (cnt.indexOf('{') != -1) {
    cnt = cnt
      ...
      .replace(/\,\"/g, ', "')
      .replace(/: {/g, ': {,   ')
      .replace(/, (Host|host|path|tls|mux|skip)/g, ',   $1');
    //console.log(cnt)
